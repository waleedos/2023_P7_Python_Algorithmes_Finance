...
def display_result(best_combination):
    ...
    print(f"\n{Fore.RED}Somme dépensée : {sum([stock[1] for stock in best_combination]) / 100}€")
    # Affiche le coût total en euros

    print(f"Profit total   : {sum([stock[2] for stock in best_combination]) / 100}€{Fore.RESET}")
...
if __name__ == "__main__":
    ...
    print(f"\nTemps d'exécution total             : {round(end_time - start_time, 3)} secondes")
    print(f"Temps pour générer les combinaisons : {round(combination_end_time - combination_start_time, 3)} secondes\n")
