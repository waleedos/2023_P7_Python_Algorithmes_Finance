import pandas as pd
# Import la bibliothèque de manipulation de données pandas sous l'alias pd, qui est  très utilisée en Python.
...
my_data = './data/dataset1_Python+P7.csv'
...
def analyse(first_result):
    ...
    # On crée une liste des noms des actions achetées.
    ...
    # Impression d'une chaîne de caractères pour introduire la liste des actions à acheter.
    print(actions)
    # Impression la liste des actions à acheter.
    print('=======================\n')
    # Impression d'une chaîne de caractères pour séparer visuellement les sections de la sortie.
...
def main():
    # Définition d'une fonction main qui englobe le flot principal de notre programme.

    df_sorted = read_data(my_data)
    # On appelle la fonction read_data avec le chemin d'accès vers notre fichier CSV en paramètre et on stocke le
    # DataFrame retourné dans df_sorted.

    bag_algorithm(df_sorted, wallet)
    # On appelle la fonction bag_algorithm avec df_sorted et wallet en paramètres.

    analyse(first_result)
    # On appelle la fonction analyse avec first_result en paramètre.
